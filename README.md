...
*   **🤝 Human-in-the-Loop (HITL) & Tool Approvals**: Enable workers to request human clarification via `ask_human` or require explicit user approval before executing custom tools. HITL interrupts are kept outside the collapsible execution steps container as standard, interactive chat bubbles (with autofocusing inline input forms) to maintain a natural, uninterrupted conversation flow.
