...
function AgentNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center gap-2 px-3 py-2 rounded-t-lg border-b border-gray-100 ${
        isRouter ? "bg-gradient-to-r from-purple-50 to-pink-50" : "bg-gradient-to-r from-indigo-50 to-purple-50"
      }`}>
        {isRouter ? <GitBranch className="w-4 h-4 text-purple-500" /> : <Brain className="w-4 h-4 text-indigo-500" />}
        <span className="font-semibold text-sm text-gray-800 truncate">{agentData.name}</span>
        <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
          isRouter ? "bg-purple-100 text-purple-600" : "bg-indigo-100 text-indigo-600"
        }`}>
      ...
      <Handle type="source" position={Position.Bottom} className={`!w-2.5 !h-2.5 ${isRouter ? "!bg-purple-500" : "!bg-indigo-500"}`} />
    </div>
  );
}
