...
class TestAttachmentEndToEnd:
    """#83: a plot generated by an agent run flows through PlotProvider into
    the unified AttachmentInstance table and is downloadable via the new
    ``GET /api/attachments/{id}`` endpoint, with the existing plot chat
    rendering (markdown image link in message content) still working."""
    ...
    async def test_agent_generated_plot_stored_and_downloadable(
        ...
        with patch(
            ...
            # Simulates the agent's turn: the ReAct loop calls generate_plot,
            # which persists an AttachmentInstance via the real repo/DB.
            provider = PlotProvider(conversation_id=conv_id, conversation_repo=repo)
            tool_result = await provider.generate_plot(
                "import matplotlib.pyplot as plt; plt.show()"
            )

        assert "/api/attachments/" in tool_result
        ...
        # The existing plot chat rendering still works: the assistant message
        # still carries the markdown image link, now pointing at /api/attachments/.
        get_resp = await authed_client.get(
            f"/api/canvases/{owned_canvas.id}/conversations/{conv_id}"
        )
        assert get_resp.status_code == 200
        assistant_msg = get_resp.json()["messages"][0]
        assert "![Plot](/api/attachments/" in assistant_msg["content"]

        attachment_id = tool_result.split("/api/attachments/")[1].rstrip(")")

        # Downloadable via the new unified endpoint.
