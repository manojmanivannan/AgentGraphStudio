...
for await (const line of readLines(Deno.stdin)) {
  ...
  try {
    // 1. Setup proper exception arguments extractor and FinalAnswer bridge
...
def final_answer(*args):
    ...
    // 2. Temporarily override stdout/stderr so we can capture prints.
    pyodide.runPython(`
import sys
import io
...
sys.stderr = buf_stderr
    ...
    // 3. Load packages — output during this phase goes to Deno's WASM stdout
    //    (Emscripten fd_write) which is NOT interceptable from JavaScript.
    //    Loading messages like "Loading numpy" will appear on Deno stdout,
    //    intermixed with our JSON response.  The Python-level Sandbox
    //    class handles this by reading multiple lines and skipping non-
    //    JSON content.
    await pyodide.loadPackagesFromImports(code);

    // 4. Run the user's code asynchronously
    const result = await pyodide.runPythonAsync(code);

    // 5. Retrieve captured stdout/stderr
    const capturedStdout = pyodide.runPython("buf_stdout.getvalue()");
    const capturedStderr = pyodide.runPython("buf_stderr.getvalue()");

    // 6. Restore original stdout/stderr
    pyodide.runPython(`
sys.stdout = old_stdout
sys.stderr = old_stderr
    `);

    // 7. Build our output object according to the rules:
